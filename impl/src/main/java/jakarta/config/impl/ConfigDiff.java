...
class ConfigDiff {
    ...
    private final Set<Key> changedKeys;

    private ConfigDiff(Config config, Set<Key> changedKeys) {
        this.config = config;
    ...
    static ConfigDiff from(Config origConfig, Config newConfig) {
        ...
        Set<Key> changedKeys = Stream.concat(forward, backward)
            .map(Config::key)
            .distinct()
            .map(Key::create)
    ...
    private static Stream<Key> expandKey(Key key) {
        Set<Key> keys = new HashSet<>();
        expandKey(key, keys);
        return keys.stream();
    }

    private static void expandKey(Key key, Set<Key> keys) {
        keys.add(key);
        if (!key.isRoot()) {
    ...
    /**
     *
     * @return the {@code Key}s that were added, removed, or the values
