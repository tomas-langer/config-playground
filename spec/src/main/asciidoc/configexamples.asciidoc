...
MicroProfile Config also provides ways to inject configured values into your beans using the `@Inject` and the `ConfigValue` qualifier.
...
When injecting a number of related configuration properties, it can be tedious to repeat the statement of `ConfigValue` in scatter places.
Since they are related, it makes more sense to aggregate them into a single property class.

MicroProfile Config provides a way to look up a number of configuration properties starting with the same prefix using the `ConfigValues` annotation, e.g. `ConfigProperties(prefix="myPrefix")`.
When annotating a class with `ConfigValues` or `@ConfigProperties(prefix="myPrefix")`, any of its fields, regardless of the visibility, maps to a configuration property via the following mapping rules.
...
In order to retrieve the above properties in a single property class, you can use the `ConfigValues` annotation with a prefix.
...
Since the class with `ConfigValues` is a CDI bean, you can use the programmatic lookup provided by CDI, e.g.
...
Specify the prefix attribute on the annotation `ConfigValues` when injecting the bean.

In this case, the prefix associated with `ConfigValues` on this injection point overrides the prefix specified on the bean class.
...
If `ConfigValues` has no associated prefix at the injection point, it defaults to the prefix set in the `Details`
class, `server`.
...
If `ConfigValues` specifies an empty prefix at the injection point:
...
** Use `ConfigValue` to provide a default value.
...
In order to avoid this, you can supply a default value when defining the field. Alternatively, you can use `ConfigValue` to provide a default value.
