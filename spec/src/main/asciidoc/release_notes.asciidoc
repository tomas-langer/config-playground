...
- Enable bulk-extraction of config properties into a separate POJO by introducing `ConfigValues` (link:https://github.com/eclipse/microprofile-config/issues/240[240])
...
- The format of the default property name for an injection point using `ConfigValue` has been changed to no longer lower case the first letter of the class.  Implementations may still support this behavior.  Instead, MicroProfile Config 1.1 requires the actual class name to be used. (link:https://github.com/eclipse/microprofile-config/issues/233[#233])
